    ...
    <div id="commercial" class="page-show">
        <div class="wc-page-top">
            <div class="top-name">
                ...
                <div class="top-title">网晨项目管理系统-商务端</div>
        ...
        <div class="wc-page-content">
            ...
            <div class="statistics">
				<div class="output" @click="toSearch()">查询项目消化情况</div>
            ...
            <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
                <van-list v-model="loadingcontent" :finished="finished" @load="onLoad">
                    ...
                    <div class="project-card-box" v-for="(projectItem,projectIndex) in projectList" :key="projectIndex">
                        <div :class="['project-card',{'over-header-light':projectItem.is_type == 2,'emergent':projectItem.hasOver,'overTime':projectItem.overTime   }]" @click.stop="showProject(projectItem.id,$event,projectIndex,projectItem.checked)">
                            <div class="card-top">
                        ...
                        <div :class="{'type-wrapper':true,'showframe':projectItem.checked,'showfalse':!projectItem.checked}" v-if="projectItem.checked">
                            <div class="type-wrapper-list-details">
                                ...
                                <div class="rate-container" v-if="projectDetailsList !=''">
                                    ...
                                    <div class="rate-info" v-if="projectDetailsList !=''">
                                        ...
                                        <van-button type="info" v-show="mainValue == 100"  @click.native="showfinishModal(projectItem.id,projectItem.user_id)">完成</van-button>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <el-dialog :visible.sync="finishModalShow" id="finishForm" :append-to-body='true'>
            <el-form>
                <el-form-item label="产品" v-show="projectFinish.cp_show">
                    <el-input type="number" v-model="projectFinish.cp_pf" @blur="scrollChange" placeholder="部门评分"></el-input>
                </el-form-item>
                <el-form-item label="设计" v-show="projectFinish.sj_show">
                    <el-input type="number" v-model="projectFinish.sj_pf" @blur="scrollChange" placeholder="部门评分"></el-input>
                </el-form-item>
                <el-form-item label="前端" v-show="projectFinish.qd_show">
                    <el-input type="number" v-model="projectFinish.qd_pf" @blur="scrollChange" placeholder="部门评分"></el-input>
                </el-form-item>
                <el-form-item label="后端" v-show="projectFinish.hd_show">
                    <el-input type="number" v-model="projectFinish.hd_pf" @blur="scrollChange" placeholder="部门评分"></el-input>
                </el-form-item>
                <el-form-item label="测试" v-show="projectFinish.cs_show">
                    <el-input type="number" v-model="projectFinish.cs_pf" @blur="scrollChange" placeholder="部门评分"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-input
                            type="textarea"
                            ...
                            placeholder="详细描述问题..."
                            v-model="projectFinish.info_details"
                            @blur="scrollChange"
                    >
                    </el-input>
                </el-form-item>
        ...
        <el-dialog :visible.sync="dialogFormVisible" :append-to-body='true'>
            <el-form ref="form" label-width="80px">
                <el-form-item label="项目群名">
                    <el-input v-model="projectInfo.name" @blur="scrollChange"></el-input>
                </el-form-item>
                <el-form-item label="立项时间">
                    <el-col :span="24">
                        <el-date-picker type="date" :editable="false" value-format="yyyy-MM-dd" placeholder="选择日期" v-model="projectInfo.lxtime" style="width: 100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
                <el-form-item label="上线时间">
                    <el-col :span="24">
                        <el-date-picker type="date" :editable="false" value-format="yyyy-MM-dd" placeholder="选择日期" v-model="projectInfo.sxtime" style="width: 100%;"></el-date-picker>
                    </el-col>
                </el-form-item>
    ...
    export default {
        ...
        data(){
            return {
                ...
                SellerList : [],
                PlanList : [],
                MoneyList : ['0-5W','5-10W','10-15W','15W以上'],
                projectInfo : {
                    name : '',
                    lxtime :  new Date(),
                    sxtime :  new Date(),
        ...
        computed : {
            ...
            //     if (search) {
            //         return this.projectList.filter(function(product) {
        ...
        methods : {
			...
			toSearch(id){
			    // localStorage.setItem('xms_id',id)
			    this.$router.replace({name:'member_table'})
			},
            proSearch(){
                let _this = this;
            ...
            showfinishModal(id,user_id){
                this.projectId = id
                this.projectDetailsList.forEach((item)=>{
                    if(item.design== '设计'){
                        this.projectFinish.sj_show = true
                    }else if(item.design == '前端'){
                        this.projectFinish.qd_show = true
                    }else if(item.design == '后端'){
                        this.projectFinish.hd_show = true
                    }else if(item.design == '产品'){
                        this.projectFinish.cp_show = true
                    }else if(item.design == '测试'){
                        this.projectFinish.cs_show = true
                    }
                })
                this.projectFinish.cp_pf = ''
                this.projectFinish.sj_pf = ''
                this.projectFinish.qd_pf = ''
                this.projectFinish.hd_pf = ''
                this.projectFinish.cs_pf = ''
            ...
            scoreSubmit(){
                ...
                this.projectDetailsList.forEach((item)=>{
                    if(item.design== '设计'){
                        this.projectFinish.sj_name = item.zx_name
                    }else if(item.design == '前端'){
                        this.projectFinish.qd_name = item.zx_name
                    }else if(item.design == '后端'){
                        this.projectFinish.hd_name = item.zx_name
                    }else if(item.design == '产品'){
                        this.projectFinish.cp_name = item.zx_name
                    }else if(item.design == '测试'){
                        this.projectFinish.cs_name = item.zx_name
                    }
                })
                let details = this.projectFinish
                let data = []
                this.projectDetailsList.forEach((item)=>{
                    let obj = {}
                    obj.uid = this.uid
                    obj.eva_id = this.projectId
                    obj.zx_post = item.design
                    obj.zx_name = item.zx_name
                    obj.m_center = details.info_details
                    obj.creat_time =  this.Change(new Date())
                    if(item.design == "设计"){
                        obj.sorce = details.sj_pf
                    }else if(item.design == "前端"){
                        obj.sorce = details.qd_pf
                    }else if(item.design == "产品"){
                        obj.sorce = details.cp_pf
                    }else if(item.design == "后端"){
                        obj.sorce = details.hd_pf
                    }else if(item.design == "测试") {
                        obj.sorce = details.cs_pf
                    }
                    data.push(obj)
                })

                if(this.projectFinish.cp_name != ''){
                    if(this.projectFinish.cp_pf == ''){
                        this.$toast({
                            message : '请填写产品部门分数!',
                            duration : 1000
                        })
                ...
                if(this.projectFinish.sj_name != ''){
                    if(this.projectFinish.sj_pf == ''){
                        this.$toast({
                            message : '请填写设计部门分数!',
                            duration : 1000
                        })
                ...
                if(this.projectFinish.qd_name != ''){
                    if(this.projectFinish.qd_pf == ''){
                        this.$toast({
                            message : '请填写前端部门分数!',
                            duration : 1000
                        })
                ...
                if(this.projectFinish.hd_name != ''){
                    if(this.projectFinish.hd_pf == ''){
                        this.$toast({
                            message : '请填写后端部门分数!',
                            duration : 1000
                        })
                ...
                if(this.projectFinish.cs_name != ''){
                    if(this.projectFinish.cs_pf == ''){
                        this.$toast({
                            message : '请填写测试部门分数!',
                            duration : 1000
                        })
                ...
                if(this.projectFinish.info_details == ''){
                        this.$toast({
                            message : '请填写项目描述!',
                            duration : 1000
                        })
                    ...
                    if(res.data.errcode == 0){
                        if(res.data.data){
                            ...
                            if(num1 +num2+num3 == 0){
                                ...
                                this.mainValue = parseInt((this.hdProject.score + this.qdProject.score + this.sjProject.score) / (num1+num2+num3))
                            }
